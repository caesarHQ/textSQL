        ...
        if (
          ...
          // Focus the map to relevant parts
            ...
            let [minLng, minLat, maxLng, maxLat] = bbox(
              turf.lineString(responseCities.map((c) => [c.long, c.lat]))
            );
          ...
          if (responseZipcodes.length == 0) {
            ...
            let [minLng, minLat, maxLng, maxLat] = bbox(
              turf.lineString(responseZipcodes.map((z) => [z.long, z.lat]))
            );
    ...
    <div className="App">
      ...
      <div className="overflow-hidden rounded-lg bg-white shadow md:h-screen">
        <div className="px-4 py-5 sm:px-6">
          ...
          <GitHubButton
            href="https://github.com/caesarhq/textSQL"
            data-icon="octicon-star"
            data-size="large"
            aria-label="Star caesarhq/textSQL on GitHub"
          >
            Github
          </GitHubButton>
          <div>
        ...
        <div className="bg-gray-50 px-4 h-full sm:p-6 flex flex-col md:flex-row md:pb-[200px]">
          <div className="rounded-lg overflow-y-scroll max-h-[60vh] h-full md:h-full md:max-h-full bg-white shadow flex-grow-[0] w-full mr-8 mb-8">
            ...
            {sql.length == 0 && !isLoading ? (
              <Examples setQuery={setQuery} handleClick={fetchBackend} />
            ) : isLoading ? (
              <> </>
            ) : (
              <>
          ...
          <div className="overflow-hidden rounded-lg bg-white shadow flex-grow-[2] h-[70vh] md:h-full w-full">
            ...
            >
              ...
              <Source
                id="zip-zoomed-out"
                type="geojson"
                data={{ type: "FeatureCollection", features: zipcodeFeatures }}
              >
              ...
              <Source
                id="cities"
                type="geojson"
                data={{ type: "FeatureCollection", features: citiesFeatures }}
              >
                <Layer {...citiesLayer} />
              </Source>
            </Map>
