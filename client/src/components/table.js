    ...
    return (
        <tbody key={Math.random()} className="divide-y divide-gray-200">
            {props.values.map((row, i) => (
                <tr key={"row" + i}>
                    {row.map((rowValue) => (
                        <td key={Math.random()} className="whitespace-nowrap py-4 pl-4 pr-3 text-sm text-left font-medium text-gray-900 sm:pl-0">
                            {rowValue}
                        </td>
                    ))}
                </tr>
                ...
                <div className="-my-2 -mx-4 overflow-x-auto sm:-mx-6 lg:-mx-8">
                    <div className="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                        <table className="min-w-full divide-y divide-gray-300">
                            <TableHeader key={Math.random()} columns={columns} />
